            ...
            <li class="flex flex-row p-1 justify-between bg-slate-100 rounded-3xl mb-3">
                <div class="my-2 mx-4">
                    <div class="flex flex-row">
                        <img class="w-10 mr-4" src="{{ asset('storage/' . $post->maskapai->images_path) ?? null }}"
                            alt="" />
                        <h3 class="text-base">{{ $post->maskapai->nama_maskapai ?? null }}</h3>
                    </div>
                ...
                <div class="my-2 mx-4">
                    <div class="flex flex-row items-center">
                        <div class="text-center">
                            <h5 class="text-lg">
                                {{ \Carbon\Carbon::parse($post->rute->keberangkatan ?? null)->hour }}.{{ \Carbon\Carbon::parse($post->rute->keberangkatan ?? null)->minute }}
                            </h5>
                            <p class="text-sm text-gray-500">{{ $post->rute->bandara_asal->kode ?? null }}</p>
                        </div>

                        <div class="text-center items-center justify-center content-center">
                            <h5 class="text-xs text-gray-500">
                                {{ \Carbon\Carbon::parse($post->rute->durasi_penerbangan ?? null)->hour }}j
                                {{ \Carbon\Carbon::parse($post->rute->durasi_penerbangan ?? null)->minute }}m</h5>
                            <div class="flex flex-row items-center">
                        ...
                        <div class="text-center">
                            <h5>{{ \Carbon\Carbon::parse($post->rute->kedatangan ?? null)->hour }}.{{ \Carbon\Carbon::parse($post->rute->kedatangan ?? null)->minute }}
                            </h5>
                            <p class="text-sm text-gray-500">{{ $post->rute->bandara_tujuan->kode ?? null }}</p>
                        </div>
                    </div>
                </div>
                <div class="my-2 mx-4">
